...
const signupValidation = [
  ...
  body("username").custom(async (value) => {
    const existingUser = await checkUserExistance(value);
    if (existingUser) {
  ...
  body("password").custom((value) => {
    console.log(value, "herererer in validation");
    if (!value[0] || !value[1]) {
      throw new Error("Both password fields are required");
    }
    if (typeof value[0] !== "string" || typeof value[1] !== "string") {
      throw new Error("Password fields must be strings");
    }
    if (value[0].trim() === "") {
...
const postSignUp = [
  ...
  async (req, res) => {
    ...
    if (!errors.isEmpty()) {
      return res
        .status(400)
        .render("signup", { pageTitle: "SignUp", errors: errors.array() });
    }
    const {
      firstName,
      lastName,
...
async function deleteMessage(req, res) {
  ...
  if (isDeleted) {
    ...
    res.send(
      "<p class='error'>message not deleted return <a href='/'>home</a></p>"
    );
  }
}
...
const postNewMsg = [
  ...
  async function (req, res) {
    ...
    if (!errors.isEmpty()) {
      return res
        .status(400)
        .render("newmsg", { pageTitle: "New Message", errors: errors.array() });
    }
